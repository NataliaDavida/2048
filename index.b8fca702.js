...
class Game {
    ...
    moveLeft() {
        ...
        for(let i = 0; i < this.state.length; i++){
            const merged = this.mergeLine([
                ...this.state[i]
            ]);
            if (merged.toString() !== this.state[i].toString()) this.moved = true;
            this.state[i] = merged;
        }
        this.afterMove();
    }
    ...
    moveUp() {
        ...
        for(let col = 0; col < 4; col++){
            ...
            for(let row = 0; row < 4; row++)if (this.state[row][col] !== merged[row]) {
    ...
    getScore() {
        ...
        if (this.gameScore) this.gameScore.innerHTML = this.score;
        return this.score;
    }
    getState() {
        return this.state;
    ...
    getStatus() {
        ...
        for(let i = 0; i < this.state.length; i++){
            for(let j = 0; j < this.state[i].length; j++)if (this.state[i][j] === 2048) {
                this.status = "win";
                for(let k = 0; k < this.messages.length; k++)if (this.messages[k].classList.contains("message-win")) this.messages[k].classList.remove("hidden");
                return "win";
            }
        }
        if (!this.canMove()) {
            this.status = "lose";
            for(let k = 0; k < this.messages.length; k++)if (this.messages[k].classList.contains("message-lose")) this.messages[k].classList.remove("hidden");
            return "lose";
        }
        this.status = "playing";
        return "playing";
    ...
    start() {
        if (this.status === "idle") {
            for(let i = 0; i < this.messages.length; i++)if (this.messages[i].classList.contains("message-start")) this.messages[i].classList.add("hidden");
            this.status = "playing";
            this.addRandomTile();
            this.addRandomTile();
            this.updateDOM();
    ...
    restart() {
        ...
        this.score = 0;
        this.status = "idle";
        for(let i = 0; i < this.messages.length; i++)if (this.messages[i].classList.contains("message-start")) this.messages[i].classList.remove("hidden");
        else this.messages[i].classList.add("hidden");
        this.getScore();
        this.updateDOM();
        this.updateButton();
    }
    ...
    updateDOM() {
        ...
        for(let i = 0; i < this.state.length; i++)for(let j = 0; j < this.state[i].length; j++){
            ...
            if (this.squares[index]) {
                this.squares[index].textContent = this.state[i][j] === 0 ? "" : this.state[i][j];
                this.addColors();
            }
        }
    }
    ...
    addColors() {
        for(let i = 0; i < this.squares.length; i++){
            ...
            if (value) this.squares[i].classList.add(`field-cell--${value}`);
        }
    }
    updateButton() {
        ...
        if (mainButton) {
            ...
            } else if (this.status === "playing" || this.status === "win" || this.status === "lose") {
